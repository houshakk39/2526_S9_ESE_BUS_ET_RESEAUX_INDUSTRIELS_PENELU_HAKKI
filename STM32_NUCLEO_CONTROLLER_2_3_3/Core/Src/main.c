...
int main(void)
{
	...
	//bmp280_read_calibration();// lecture en bloc 0x88..0xA1
	...
	while (1)
	{
		...
		// Valeurs 32 bits non compensées
		printf("raw_temp = %lu, raw_press = %lu\r\n",
				(unsigned long)raw_temp,
				(unsigned long)raw_press);

		HAL_Delay(500); // 2 mesures par seconde
		/* USER CODE END WHILE */
